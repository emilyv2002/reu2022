'''
Tube Extraction for UCF-MAMA: Training Videos 
Created by Emily Vo
'''
...
data_dir = '/home/rmodi/crcv_work/tiny_dataset_creation/final_dataset/tiny_detections_cvpr_2022/to_release/train'
dump_path = '/home/em585489/mama_dataloader/tubes'
...
def generate_crops(clip, bbox, start, end, t_len, actors_involved):
    ...
    if len(actors_involved) == 1:
        for f_id in range(start, end):   #(start, end+1)
            if bbox_idx > len(bbox):
                break
            frame = clip[f_id, :, :, :]
            b_box = bbox[bbox_idx]
            x1, y1, x2, y2 = b_box
            ...
            # print('this is first element of shape', cropped_frame.shape[0])
            # exit()
...
def extract_tubes(clip, vid_id, label_ids, dump_path='/home/em585489/mama_dataloader/tubes', tube_len=100):
    ...
    for action_id in ann.keys():     # was list(ann,keys())
        ...
        for frame_id in detections_dict.keys():
            ...
            if len(actors_involved) > 1:    # if not len(actors_involved) <= 1:
